...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      '@cell-diagrams/language-server/browser': resolve(__dirname, '../../packages/language-server/src/browser/index.ts'),
    },
  },
  optimizeDeps: {
    // Exclude workspace packages from pre-bundling (they're aliased to source)
    exclude: ['@cell-diagrams/core', '@cell-diagrams/renderer', '@cell-diagrams/collab-client', '@cell-diagrams/language-server'],
    include: ['vscode-languageserver-protocol', 'vscode-languageserver-protocol/browser'],
  },
  worker: {
    format: 'es',
  },
